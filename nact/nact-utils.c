...
gchar* nact_utils_parse_parameter (void)
{
	...
	tmp = g_string_free (tmp_string, FALSE); // return the content of the GString
	
	// Convert special xml chars (&, <, >,...) to avoid warnings generated by Pango parser
	encoded_string = xmlEncodeSpecialChars (NULL, tmp);

	g_free (tmp);

	retv = xmlStrdup (encoded_string);

	xmlFree (encoded_string);
