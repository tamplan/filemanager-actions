...
/* i18n notes : Sorry for this long paragraph, will try to fix it in the next release */
#define ACTION_SCHEMES_DESC_LONG		_("Defines the list of valid GnomeVFS schemes to be matched with the selected items. The GnomeVFS scheme is the protocol used to access the files. The keyword to use is the one used in the GnomeVFS URI.\n\nExample of GnomeVFS URI : \nfile:///tmp/foo.txt\nsftp:///root@test.example.net/tmp/foo.txt\n\nThe most common schemes are :\n\n'file' : local files\n'sftp' : files accessed via SSH\n'ftp' : files accessed via FTP\n'smb' : files accessed via Samba (Windows share)\n'dav' : files accessed via WebDav\n\nAll GnomeVFS schemes used by Nautilus can be used here.")
